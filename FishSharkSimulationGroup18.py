...
def BoundaryRepulsion(center_x, center_y, margin, repulsion_strength, x, y,vx,vy, field_size):
    ...
    if distance > boundary_radius - margin:
        ...
        vx -= direction_x * repulsion_strength * (distance - (boundary_radius - margin)) 
        vy -= direction_y * repulsion_strength * (distance - (boundary_radius - margin)) 
...
class Shark:
    ...
    def move(self, fish_population, fish_position_x, fish_position_y):
        ...
        if closest_fish:
            ...
            if USE_DELAY:
                ...
                if dist > 0:
                    ...
                    self.x += 0.2*dx*self.vx/dist + 0.8*(dx1 / dist1) * self.vx
                    self.y += 0.2*dy*self.vy/dist + 0.8*(dy1 / dist1) * self.vy
            else:
        ...
        #looks neater!)
        self.vx, self.vy = BoundaryRepulsion(FIELD_SIZE/2, FIELD_SIZE/2, 0.1*FIELD_SIZE, 20, self.x, self.y,self.vx,self.vy, FIELD_SIZE)
