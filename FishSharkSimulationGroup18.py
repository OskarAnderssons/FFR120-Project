...
SENSORY_DELAY_SHARK = -10 #Placeholder value for now -2 or lower if USE_DELAY == TRUE
        ...
        if closest_fish:
            ...
            if USE_DELAY:
                ...
                if dist > 0:
                    ...
                    self.x += 0.4*dx*self.vx/dist + 0.6*(dx1 / dist1) * self.vx
                    self.y += 0.4*dy*self.vy/dist + 0.6*(dy1 / dist1) * self.vy
            else:
