...
#Tuneable Parameters
NUM_FISH = 30 #Amount of prey constant for now
BASE_COHESION = 0.5
NUM_SHARKS = 3 #Amount of predators
FIELD_SIZE = 800 #Size of area, also affects simulation windowsize!
...
PREDATOR_COOLDOWN = 100  #Cooldown for predator chasing and eating
...
class Shark:
    ...
    def move(self, fish_population, fish_position_x, fish_position_y):
        if self.cooldown > 0:
            ...
            if distance > 10:
                self.vx = (dx / distance) * 0.3 * PREDATOR_SPEED  # Slower return during cooldown
                self.vy = (dy / distance) * 0.3 * PREDATOR_SPEED
            else:
                self.vx = 0
                self.vy = 0
                self.cooldown = 0
...
class FishSimulation:
    def __init__(self, root, nosim):
        ...
        for fish in self.fish_population:
            ...
            radius = i * (2 + 1 * randomNumber)
            dir = i * (math.pi / 5 + 2 * randomNumber)
            fish.x = xx + radius * math.cos(dir)
            fish.y = yy + radius * math.sin(dir)
            #fish.vx = -2 * randomNumber
            #fish.vy = FISH_SPEED
            fish.vx = FISH_SPEED
            fish.vy = 0
            #fish.vx = FISH_SPEED / 10 * math.cos(dir)
            #fish.vy = FISH_SPEED / 10 * math.sin(dir)
            i += 1
    ...
    def updateCanvas(self):
        ...
        for shark in self.sharks:
            self.canvas.create_oval(
                (shark.x - PREDATOR_VISION) * self.scaler, (shark.y - PREDATOR_VISION) * self.scaler, (shark.x + PREDATOR_VISION)*self.scaler, (shark.y + PREDATOR_VISION)*self.scaler, outline="blue"
            )
...
def runSystem(iterations):
    ...
    for itr in range(iterations):
        ...
        for i,fish in enumerate(fish_population):
            fish.move(fish_population, sharks)
            fish_position_x[i,:] = np.roll(fish_position_x[i,:], -1, axis=0)
            fish_position_y[i,:] = np.roll(fish_position_y[i,:], -1, axis=0)
            xpos, ypos = fish.getFishPosition()
            fish_position_x[i,DELAY_TIME-1] = xpos
        ...
        for shark in sharks:
            shark.move(fish_population, fish_position_x, fish_position_y)
            fish_eaten = shark.eat(fish_population)
            
        total_fish_eaten1 += fish_eaten
        
        avg_cohesion = sum(fish.cohesion for fish in fish_population) / len(fish_population) if fish_population else 0 #Not used
        num_fish_alive = len(fish_population)
            

        avg_fish_eaten_per_step = total_fish_eaten1 /time_elapsed1 if time_elapsed1 > 0 else 0
        
        if len(fish_population) != NUM_FISH:
            fish_population.append(Fish(BASE_COHESION))
                
        time_elapsed1 += 1
    
    return avg_fish_eaten_per_step
